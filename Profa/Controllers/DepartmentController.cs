...
namespace Profa.Controllers
{
    ...
    public class DepartmentController : ControllerBase
    {
        ...
        [HttpGet("{id}")]
        public async static Task<List<Department>> GetDepartmentsByBranchId(int id)
        {
            using (var dataContext = new ProfaContext())
            {
                return await dataContext.Departments.Where(Department => Department.BranchId == id).ToListAsync(); 
            }
        }
