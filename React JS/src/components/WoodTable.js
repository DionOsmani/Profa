...
return (
    ...
    getWood();
        ...
        {(woods.length>0 && showingCreateNewWoodForm === false && woodCurrentlyBeingUpdated === null) && (
        ...
        {showingCreateNewWoodForm && (
        localStorage.getItem('Role')==="Admin" ||
        localStorage.getItem('Role')==="HeadAdmin"  ||
        localStorage.getItem('Role')==="bHead" ||
        localStorage.getItem('Role')==="dHead" ||
        localStorage.getItem('Role')==="worker")  
        && <WoodCreateForm onWoodCreated={onWoodCreated}/>}

        {woodCurrentlyBeingUpdated!==null && (
        localStorage.getItem('Role')==="Admin" ||
        localStorage.getItem('Role')==="HeadAdmin"  ||
        localStorage.getItem('Role')==="bHead" ||
        localStorage.getItem('Role')==="dHead" ||
        localStorage.getItem('Role')==="worker")  
        && <WoodUpdateForm wood={woodCurrentlyBeingUpdated} onWoodUpdated={onWoodUpdated} />}
    </div>
  )
...
function renderWoodTable(){
    return(
        ...
        <button onClick={() => setShowingCreateNewWoodForm(true)} className='btn btn-outline-dark btn-lg w-25 float-right m-1'>Create new wood</button>
            ...
            <tbody>
                {woods.map((wood) => (
                    <tr key={wood.woodId}>
                        ...
                        <td>{wood.amount}</td>
                        <td>{wood.branchId}</td>
                        <td><button onClick={() => setWoodCurrentlyBeingUpdated(wood) } className='btn btn-dark btn-lg mx-3 my-3'>Update</button></td>
                        <td><button onClick={() => {if(window.confirm(`Are you sure you want to delete this wood?`)) deleteWood(wood.woodId)}} className='btn btn-secondary btn-lg mx-3 my-3'>Delete</button></td>
                    </tr>
                ))}
            </tbody>
        </table>
