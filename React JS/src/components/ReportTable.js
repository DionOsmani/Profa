...
return (
    ...
    getReport();
        ...
        {(reports.length>0 && showingCreateNewReportForm === false && reportCurrentlyBeingUpdated === null) && localStorage.getItem('token') && (
        ...
        {showingCreateNewReportForm && localStorage.getItem('token') && (
        localStorage.getItem('Role')==="Admin" ||
        localStorage.getItem('Role')==="HeadAdmin"  ||
        localStorage.getItem('Role')==="bHead" ||
        localStorage.getItem('Role')==="dHead" ||
        localStorage.getItem('Role')==="worker")  
        && <ReportCreateForm onReportCreated={onReportCreated}/>}

        {reportCurrentlyBeingUpdated!==null && localStorage.getItem('token') && (
        localStorage.getItem('Role')==="Admin" ||
        localStorage.getItem('Role')==="HeadAdmin"  ||
        localStorage.getItem('Role')==="bHead" ||
        localStorage.getItem('Role')==="dHead" ||
        localStorage.getItem('Role')==="worker")  
        && <ReportUpdateForm report={reportCurrentlyBeingUpdated} onReportUpdated={onReportUpdated} />}
    </div>
  )
...
function renderReportTable(){
    return(
        ...
        <button onClick={() => setShowingCreateNewReportForm(true)} className='btn btn-outline-dark btn-lg w-25 float-right m-1'>Create new report</button>
            ...
            <tbody>
                {reports.map((report) => (
                    <tr key={report.reportId}>
                        ...
                        <td><button onClick={() => setReportCurrentlyBeingUpdated(report) } className='btn btn-dark btn-lg mx-3 my-3'>Update</button></td>
                        {localStorage.getItem('token') && (
                        localStorage.getItem('Role')==="Admin" ||
                        localStorage.getItem('Role')==="HeadAdmin"  ||
                        localStorage.getItem('Role')==="bHead" ||
                        localStorage.getItem('Role')==="dHead")  
                        && <td><button onClick={() => {if(window.confirm(`Are you sure you want to delete this report?`)) deleteReport(report.reportId)}} className='btn btn-secondary btn-lg mx-3 my-3'>Delete</button></td>}
                    </tr>
                ))}
            </tbody>
        </table>
