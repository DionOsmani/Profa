...
return (
    ...
    getTool();
        ...
        {(tools.length>0 && showingCreateNewToolForm === false && toolCurrentlyBeingUpdated === null) && localStorage.getItem('token') && (
        ...
        {showingCreateNewToolForm && localStorage.getItem('token') && (
        localStorage.getItem('Role')==="Admin" ||
        localStorage.getItem('Role')==="HeadAdmin"  ||
        localStorage.getItem('Role')==="bHead" ||
        localStorage.getItem('Role')==="dHead")  
        && <ToolCreateForm onToolCreated={onToolCreated}/>}

        {toolCurrentlyBeingUpdated!==null && localStorage.getItem('token') && (
        localStorage.getItem('Role')==="Admin" ||
        localStorage.getItem('Role')==="HeadAdmin"  ||
        localStorage.getItem('Role')==="bHead" ||
        localStorage.getItem('Role')==="dHead" ||
        localStorage.getItem('Role')==="worker")  
        && <ToolUpdateForm tool={toolCurrentlyBeingUpdated} onToolUpdated={onToolUpdated} />}
    </div>
  )
...
function renderToolTable(){
    return(
        ...
        localStorage.getItem('Role')==="bHead" ||
        localStorage.getItem('Role')==="dHead")  
        &&<button onClick={() => setShowingCreateNewToolForm(true)} className='btn btn-outline-dark btn-lg w-25 float-right m-1'>Create new tool</button>}
            ...
            <tbody>
                {tools.map((tool) => (
                    <tr key={tool.toolId}>
                        ...
                        <td><button onClick={() => setToolCurrentlyBeingUpdated(tool) } className='btn btn-dark btn-lg mx-3 my-3'>Update</button></td>
                        {localStorage.getItem('token') && (
                        localStorage.getItem('Role')==="Admin" ||
                        localStorage.getItem('Role')==="HeadAdmin"  ||
                        localStorage.getItem('Role')==="bHead" ||
                        localStorage.getItem('Role')==="dHead")  
                        &&<td><button onClick={() => {if(window.confirm(`Are you sure you want to delete this tool?`)) deleteTool(tool.toolId)}} className='btn btn-secondary btn-lg mx-3 my-3'>Delete</button></td>}
                    </tr>
                ))}
            </tbody>
        </table>
