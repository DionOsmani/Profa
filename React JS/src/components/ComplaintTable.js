...
return (
    ...
    getComplaint();
        ...
        {(complaints.length>0 && showingCreateNewComplaintForm === false && complaintCurrentlyBeingUpdated === null) && localStorage.getItem('token')&&(
        ...
        {showingCreateNewComplaintForm && localStorage.getItem('Role')=="Admin" &&(
        localStorage.getItem('Role')==="Admin" ||
        localStorage.getItem('Role')==="HeadAdmin"  ||
        localStorage.getItem('Role')==="bHead" ||
        localStorage.getItem('Role')==="dHead" ||
        localStorage.getItem('Role')==="worker")  
        && <ComplaintCreateForm onComplaintCreated={onComplaintCreated}/>}

        {complaintCurrentlyBeingUpdated!==null && localStorage.getItem('Role')=="Admin" &&(
        localStorage.getItem('Role')==="Admin" ||
        localStorage.getItem('Role')==="HeadAdmin"  ||
        localStorage.getItem('Role')==="bHead" ||
        localStorage.getItem('Role')==="dHead" ||
        localStorage.getItem('Role')==="worker")  
        && <ComplaintUpdateForm complaint={complaintCurrentlyBeingUpdated} onComplaintUpdated={onComplaintUpdated} />}
    </div>
  )
...
function renderComplaintTable(){
    return(
        ...
        <div className='w-75 mx-auto'>
            {console.log("test")}
        <button onClick={() => setShowingCreateNewComplaintForm(true)} className='btn btn-outline-dark btn-lg w-25 float-right m-1'>Create new complaint</button>
            ...
            <tbody>
                {complaints.map((complaint) => (
                    <tr key={complaint.complaintId}>
                        ...
                        <td><button onClick={() => setComplaintCurrentlyBeingUpdated(complaint) } className='btn btn-dark btn-lg mx-3 my-3'>Update</button></td>
                        {localStorage.getItem('token') &&(
                        localStorage.getItem('Role')==="Admin" ||
                        localStorage.getItem('Role')==="HeadAdmin")  
                        && <td><button onClick={() => {if(window.confirm(`Are you sure you want to delete this complaint?`)) deleteComplaint(complaint.complaintId)}} className='btn btn-secondary btn-lg mx-3 my-3'>Delete</button></td>}
                    </tr>
                ))}
            </tbody>
        </table>
